...
const ManageBooking = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Use the LIVE Duffel retrieval endpoint
      ...
      if (res.data.success && res.data.data) {
         ...
         toast.success('Xogtaada waa la helay!');
      } else {
         toast.error('Cillad ayaa dhacday, fadlan isku day hadane.');
      }
    } catch (err) {
      const msg = err.response?.data?.message || 'Booking-ka lama helin ama Last Name-ka waa khalad.';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10 pt-10">
          ...
          <p className="text-gray-500 font-medium">Xaqiiji duulimaadkaaga adoo isticmaalaya PNR iyo Last Name</p>
        </div>
        ...
        {booking ? (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
             ...
             <div className="bg-white rounded-[2.5rem] shadow-2xl border border-indigo-50 overflow-hidden">
                <div className="bg-gray-900 p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                   <div className="flex gap-6 items-center">
                      ...
                      <div>
                         ...
                         <h2 className="text-4xl font-black">{booking?.booking_reference || 'N/A'}</h2>
                      </div>
                   </div>
                ...
                <div className="p-8 md:p-12 space-y-12">
                   ...
                   <div className="space-y-12 border-l-4 border-dashed border-gray-100 ml-4 md:ml-8 pl-8 md:pl-12 relative">
                      {booking?.slices?.length > 0 ? booking.slices.map((slice, sIdx) => (
                        <div key={sIdx} className="space-y-12 pb-8">
                                ...
                                <div className="flex flex-col lg:flex-row justify-between gap-8">
                                   <div className="flex-1">
                                      <div className="flex items-center gap-4 mb-4">
                                         ...
                                         <span className="font-black text-gray-900">{segment?.operating_carrier?.name || 'Airline'} — {segment?.operating_carrier_flight_number || ''}</span>
                                      </div>
                         ...
                         <div className="p-8 text-center text-gray-400 font-bold">Flight itinerary information is currently not available.</div>
                      )}
                   </div>
                   ...
                   <div className="pt-8 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div>
                         ...
                         <div className="space-y-3">
                            ...
                            )) || <p className="text-gray-400">No passenger data found.</p>}
                         </div>
                      </div>
                      <div className="bg-indigo-600 rounded-3xl p-8 text-white flex flex-col justify-center items-center text-center shadow-xl shadow-indigo-200">
                         ...
                         <p className="text-5xl font-black tracking-tighter mb-6">{booking?.total_currency} {booking?.total_amount || '0.00'}</p>
                         <button 
                           onClick={() => window.print()} 
                           className="w-full py-4 bg-white text-indigo-600 font-black rounded-2xl hover:bg-indigo-50 transition-colors shadow-lg"
                         >
                            Print Trip Details
                         </button>
                      </div>
           ...
           !loading && <div className="text-center p-20 bg-white rounded-[3rem] border-2 border-dashed border-gray-100 text-gray-300 font-black text-lg">Xogtaadu halkan ayay ka soo muuqan doontaa...</div>
        )}
      </div>
    </div>
  );
